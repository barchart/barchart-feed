...
public class SeriesSubscription implements Subscription {
    ...
    public SeriesSubscription(final String symbol, final Instrument i, final String analyticSpecifier, final TimeFrame[] timeFrames, final TradingWeek week) {
    	this.instrument = i;
    	this.symbol = symbol;
    ...
    public SeriesSubscription(final SeriesSubscription ss) {
        ...
        for(final TimeFrame tf : ss.timeFrames) {
            timeFrames[i++] = tf;
        }
	 ...
	 * @param tf
	 */
	public void setTimeFrames(final TimeFrame[] tf) {
	    this.timeFrames = tf;
	}
	 ...
	 * @param tf
	 */
	public void addTimeFrame(final TimeFrame tf) {
	    final TimeFrame[] array = new TimeFrame[timeFrames.length + 1];
	    System.arraycopy(timeFrames, 0, array, 0, timeFrames.length);
	    array[array.length - 1] = tf;
	    this.timeFrames = array;
	}

	public TimeFrame getTimeFrame(final int index) {
	 ...
	 * @param specifier
	 */
	public void setAnalyticSpecifier(final String specifier) {
     ...
     */
	@Override
    public boolean isDerivableFrom(final Subscription otherSubscription) {
		boolean retVal = false;
		final SeriesSubscription other = (SeriesSubscription)otherSubscription;
	 ...
	 */
	public boolean isCloserTo(final Subscription target, final Subscription comparison) {
	 ...
	 */
	public void loadFromNodeDescriptor(final String sourceKey, final SeriesSubscription nextSubscription, final AnalyticNodeDescriptor desc) {
        final AnalyticNodeDescriptor id = desc.getInputNodeDescriptor(sourceKey);
        if (id == null)
        ...
        final String[] timeframes = desc.getInputTimeframes(sourceKey);
        if ((timeframes == null) || (timeframes.length == 0)) {
            this.addTimeFrame(nextSubscription.getTimeFrame(0));
        } else {
            for (final String timeframe: timeframes) {
                final int index = desc.getTimeframeIndex(timeframe);
                if (index > -1) {
	...
	public boolean equals(final Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		final SeriesSubscription other = (SeriesSubscription) obj;
	...
	public String toString() {
	    final StringBuilder sb = new StringBuilder("[ ").append(analyticSpecifier).append(" ").append(symbol).append(" ");
